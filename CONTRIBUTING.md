...
1.  **Mobile-First**: All CSS begins with mobile styles. `@media (min-width)` queries are used *exclusively* to add complexity for larger screens.
2.  **Zero-Dependency**: We avoid libraries (Bootstrap, jQuery) unless absolutely necessary.
3.  **Semantic HTML**: Use correct tags (`<nav>`, `<article>`, `<section>`) for maximum accessibility.
...
**Example**:
```bash
git commit -m "feat: implement dark mode toggle using local storage"
git commit -m "fix(mobile): resolve menu background color in dark mode"
```
